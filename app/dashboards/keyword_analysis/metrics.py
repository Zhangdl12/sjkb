...
def build_total(df: pd.DataFrame, config: AppConfig) -> pd.DataFrame:
    ...
    result_df[config.keyword_category_column] = "总计"
    result_df[config.keyword_second_category_column] = ""
    result_df[config.keyword_column] = ""
    result_df[config.month_label_column] = ""
    result_df["花费占比%"] = 1.0
...
def _build_classification_summary(
    ...
    result_df = _append_classification_ratio_metrics(grouped_df, config)
    return result_df
