...
from app.dashboards.keyword_analysis.tree_builder import (
    build_classification_tree_summary,
    build_time_tree_summary,
)


KeywordAnalysisView = Literal["词性汇总", "时间渠道"]
KEYWORD_ANALYSIS_PAYLOAD_CACHE_VERSION = "keyword_analysis_v2_month_tree"
...
class KeywordAnalysisPayload:
    ...
    Attributes:
        classification_df: 词性汇总树表，仅词性汇总视图使用。
        time_df: 时间渠道树表，仅时间渠道视图使用。
        total_df: 当前筛选范围总计表，仅词性汇总视图使用。
    """
...
def load_keyword_analysis_dataset(
    _source_bytes: bytes,
    source_name: str,
    source_token: str,
    _tag_bytes: bytes,
    ...
    tag_tables = load_excel_sheets(
        ...
        _build_tag_usecols(_tag_bytes, config),
    )
    return build_keyword_analysis_dataset({**source_tables, **tag_tables}, config)


@st.cache_data(show_spinner=False)
def load_keyword_analysis_payload(
    _source_bytes: bytes,
    source_name: str,
    source_token: str,
    _tag_bytes: bytes,
    ...
    view_type: KeywordAnalysisView,
    payload_version: str = KEYWORD_ANALYSIS_PAYLOAD_CACHE_VERSION,
    ...
    _ = payload_version
    config = AppConfig()
    dataset = load_keyword_analysis_dataset(
        _source_bytes,
        source_name,
        source_token,
        _tag_bytes,
...
def build_filter_fields(config: AppConfig, df: pd.DataFrame | None = None) -> tuple[FilterField, ...]:
    ...
    Args:
        config: 关键词分析字段配置。
        df: 可选的关键词分析明细，用于判断店铺级特殊字段是否存在有效值。
    ...
    fields = [
        ...
        FilterField(config.keyword_category_column, group="关键词"),
        FilterField(config.keyword_column, group="关键词"),
    ...
    optional_fields = [
        FilterField(config.keyword_second_category_column, group="关键词"),
    ...
    if df is None:
        ...
        if _has_effective_values(df, config.keyword_second_category_column):
            fields.insert(6, optional_fields[0])
        fields.extend(field for field in optional_fields[1:] if _has_effective_values(df, field.column))
    return tuple(fields)
...
def _build_tag_usecols(_tag_bytes: bytes, config: AppConfig) -> dict[str, list[str]]:
    ...
    keyword_columns = sheet_columns.get(config.keyword_tag_sheet, [])
    if config.keyword_second_category_column in keyword_columns:
        usecols["keyword_tag"].append(config.keyword_second_category_column)
...
def _resolve_dataset(tables: dict[str, pd.DataFrame], config: AppConfig) -> pd.DataFrame:
    ...
    return build_keyword_analysis_dataset(tables, config)


def _empty_payload(config: AppConfig) -> KeywordAnalysisPayload:
