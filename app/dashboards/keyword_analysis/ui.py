...
DIMENSION_COLUMNS = {"词性分类", "词性二级分类", "关键词", "月", "周", "日"}
...
def render_classification_table(title: str, df: pd.DataFrame, key: str) -> None:
    ...
    st.markdown(f"### {title}")
    if df.empty:
        render_empty_state("当前筛选条件下没有可展示的词性汇总数据。")
        return
    _render_aggrid(
        df,
        _build_tree_grid_options(
            df,
            hidden_columns={"path", "词性分类", "词性二级分类", "关键词", "月"},
            group_header="词性分类 > 词性二级分类 > 关键词 > 月",
        ),
        key,
    )
...
def render_time_table(title: str, df: pd.DataFrame, key: str) -> None:
    ...
    _render_aggrid(
        ...
        _build_tree_grid_options(
            df,
            hidden_columns={"path", "词性分类", "关键词", "周", "日"},
            group_header="词性分类 > 关键词 > 周 > 日",
        ),
        key,
    )
...
def render_total_table(df: pd.DataFrame) -> None:
    ...
    if df.empty:
        ...
        return
    _render_aggrid(df, _build_plain_grid_options(df), "keyword_analysis_total_grid", height=110)


def render_empty_state(message: str = "当前数据为空，无法展示关键词分析表。") -> None:
    """渲染空结果提示。
...
def _build_column_def(column: str, hide: bool = False) -> dict:
    ...
    if column in PERCENT_COLUMNS:
        # 百分比主表保持紧凑展示，极小值自动增加小数位；悬停时展示更完整精度。
        column_def.update(
            {
                "type": "numericColumn",
                "width": 130,
    ...
    if column in DIMENSION_COLUMNS and column in {"词性分类", "关键词"}:
