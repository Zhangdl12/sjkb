"""CPS 分析页面 AgGrid 渲染函数。"""
import pandas as pd
import streamlit as st
from st_aggrid import AgGrid, DataReturnMode, GridUpdateMode, JsCode


PERCENT_COLUMNS = {"佣金比例"}
AMOUNT_COLUMNS = {"tk计佣金额", "tk总佣金"}
DIMENSION_COLUMNS = {"所属计划/活动", "日", "商品名称"}
...
def render_cps_tree_table(title: str, df: pd.DataFrame, key: str) -> None:
    """渲染“团长 > 日期 > 产品”树形表。

    Args:
        title: 表格标题。
        df: 带 path 列的 CPS 汇总表。
    ...
    st.markdown(f"### {title}")
    if df.empty:
        render_empty_state("当前筛选条件下没有可展示的 CPS 汇总数据。")
        return
    _render_aggrid(
        df,
        _build_tree_grid_options(
            df,
            hidden_columns={"path", "所属计划/活动", "日", "商品名称"},
...
def render_empty_state(message: str = "当前数据为空，无法展示 CPS 分析表。") -> None:
    """渲染空结果提示。
...
def _build_tree_grid_options(df: pd.DataFrame, hidden_columns: set[str], group_header: str) -> dict:
    ...
    return {
        ...
        "columnDefs": [
            _build_column_def(column, hide=column in hidden_columns)
...
def _build_column_def(column: str, hide: bool = False) -> dict:
    ...
    if column in PERCENT_COLUMNS:
        # 佣金比例按小数存储，页面展示为百分比，tooltip 提供更完整精度便于核对。
        column_def.update(
            {
                "type": "numericColumn",
                "width": 130,
        ...
        # 金额列保留两位以内小数并加千分位，适合直接和 Excel 透视表核对。
        column_def.update(
            {
                "type": "numericColumn",
                "width": 140,
                "valueFormatter": THOUSAND_FORMATTER,
                "tooltipValueGetter": NUMBER_TOOLTIP,
            }
        )
    if column in DIMENSION_COLUMNS:
        column_def["minWidth"] = 160
    return column_def
